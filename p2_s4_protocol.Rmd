...
Participants were randomly assigned by Qualtrics to the peer prime group or the leader prime group. 
...
- Participants were randomly assigned to the Peer prime group or the Leader prime group
...
for(i in 1:nrow(data)){
        ...
        ##############################
        #sum up answers Pietra. method
        ##############################
        
        #loop through and categorize all 8 answers
        
        for(j in 1:8){
        #correct: response (face) = randomiz matrix (face)
        if(data[i, j] == matr$face[matr$sen == j]) { 
                q[i, j] <- "correct" 
        #within within category:response (face - trait) = randomiz matrix. on both traits
        } else if(matr$comp[matr$face == data[i, j]] == matr$comp[matr$sen == j] & 
                  matr$warm[matr$face == data[i, j]] == matr$warm[matr$sen == j]){
                q[i, j] <- "within_within"
        #within between category: match on competence but not on warmth
        } else if(matr$comp[matr$face == data[i, j]] == matr$comp[matr$sen == j] & 
                  matr$warm[matr$face == data[i, j]] != matr$warm[matr$sen == j]){
                q[i, j] <- "within_between"
        #between within category: match on warmth but not on competence
        } else if(matr$comp[matr$face == data[i, j]] != matr$comp[matr$sen == j] & 
                  matr$warm[matr$face == data[i, j]] == matr$warm[matr$sen == j]){
                q[i, j] <- "between_within"
        #between_between: all other :) 
        } else  p_warm[i, j] <- "between_between"
}
        
        data$within_within[i] <- sum(q[i, ] == "within_within", na.rm =  T)
        data$between_within[i] <- sum(q[i, ] == "between_within", na.rm =  T)
        data$within_between[i] <- sum(q[i, ] == "within_between", na.rm =  T)
        data$between_between[i] <- sum(q[i, ] == "between_between", na.rm =  T)
} 
...
cat("is the competence categorization score significantly different from 0 in the whole sample?")
...
cat("is the competence categorization score significantly different from 0 in the peer prime group?")
...
cat("is the competence categorization score significantly different from 0 in the leader prime group?")
...
cat("Does the treatment increase competence categorization?")
...
cat("is the likability categorization score significantly different from 0 in the whole sample?")
...
cat("is the likability categorization score significantly different from 0 in the peer prime group?")
...
cat("is the  likability categorization score significantly different from 0 in the leader prime group?")
...
cat("Does the treatment increase likability categorization?")
treateff <- t.test(data$warm_catscore ~ data$treat)
pander(treateff)
t2r(treateff$statistic, treateff $parameter)

cat("In the leader frame, is categorization by competence significantly higher than by likability?")
pander(t.test(data$warm_catscore[data$treat == "Lead prime"],
...
######################################
# correct for base raits Pietr. method
######################################

data$within_between <- data$within_between * 0.5
data$between_within <- data$between_within * 0.5
data$between_between <- data$between_between * 0.5

data$comp_pietr_catscore <- (data$within_within + data$within_between) - 
        (data$between_within + data$between_between)

data$warm_pietr_catscore <- (data$within_within + data$between_within) - 
        (data$within_between + data$between_between)

# pietr catscore results competence
t.test(data$comp_pietr_catscore[data$treat == "Lead prime"])
r_effsize(data$comp_pietr_catscore[data$treat == "Lead prime"])
t.test(data$comp_pietr_catscore[data$treat == "Peer prime"])
r_effsize(data$comp_pietr_catscore[data$treat == "Peer prime"])

t.test(data$comp_pietr_catscore ~ data$treat)
r_effsize(data$comp_pietr_catscore ~ data$treat)

# pietr catscore results warmth
t.test(data$warm_pietr_catscore[data$treat == "Lead prime"])
r_effsize(data$warm_pietr_catscore[data$treat == "Lead prime"])
t.test(data$warm_pietr_catscore[data$treat == "Peer prime"])
r_effsize(data$warm_pietr_catscore[data$treat == "Peer prime"])

t.test(data$warm_pietr_catscore ~ data$treat)
r_effsize(data$warm_pietr_catscore ~ data$treat)
...
# Source R code for vertical dodging. 
source("https://raw.githubusercontent.com/jaredlander/coefplot/master/R/position.r")
...
ggsave("/Users/szasulja/Google Drive/aarhus/Paper 2/paper2_github/Figure2.pdf", width = 6, height = 2, units = "in")

cat("Partner / Leader choice")
...
cat("Alternative approach: considering trait combinations")
...
cat("These effects are not driven by a single outlier action sentence")
...
- Replication data set is available through https://raw.githubusercontent.com/alexanderbor/paper2/master/P2_S4.csv
